...
const TradeControl = () => {
  ...
  useEffect(() => {
    const fetchClients = async () => {
      try {
        ...
        // Sort clients with active trades at the top
        clientsWithActiveTrades.sort(
          (a, b) => b.hasActiveTrades - a.hasActiveTrades
        );
        setClients(clientsWithActiveTrades);
        setFilteredClients(clientsWithActiveTrades); // Initialize filteredClients
      } catch (error) {
  ...
  useEffect(() => {
    if (userId) {
      fetchPredictions(userId); // Fetch predictions when the component mounts if userId is present
    } else {
  ...
  const handleDefaultControl = async (result) => {
    try {
      // Set the defaultTradeResult to "null" when "default" is selected
